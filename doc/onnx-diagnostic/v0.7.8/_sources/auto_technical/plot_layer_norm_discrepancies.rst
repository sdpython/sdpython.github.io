...
.. rst-class:: sphx-glr-script-out
 ...
 .. code-block:: none
    ...
    -- running with torch
    -- torch: #1[CT1s2x1024x1152[238.43856811523438,325.49493408203125:A282.03615981600404]]
    -- running with ort
    -- ort: #1[A1s2x1024x1152[238.43858337402344,325.4949645996094:A282.0361598214626]]
    -- diff abs=0.00018310546875, rel=6.353993180060876e-07, n=2359296.0/#1465821>0.0-#1629>0.0001

    -- running on CPU with FLOAT16
    -- running with torch
    -- torch: #1[CT10s2x1024x1152[238.375,325.5:A282.03590652677747]]
    -- running with ort
    -- ort: #1[A10s2x1024x1152[238.5,325.5:A282.0361620585124]]
    -- diff abs=0.25, rel=0.0009765586853176355, n=2359296.0/#592527>0.0-#592527>0.0001-#592527>0.001-#592527>0.01-#592527>0.1

    -- running on CUDA with FLOAT
    -- running with torch
    -- torch: #1[GT1s32x1024x1152[230.6787109375,331.31390380859375:A282.099824885446]]
    -- running with ort
    -- ort: #1[A1s32x1024x1152[230.67393493652344,331.3173828125:A282.09724871351983]]
    -- diff abs=0.019500732421875, rel=6.741399644447511e-05, n=37748736.0/#37645799>0.0-#37025987>0.0001-#31072720>0.001-#437722>0.01

    -- running on CUDA with FLOAT16
    -- running with torch
    -- torch: #1[GT10s32x1024x1152[230.625,331.25:A282.0973633262846]]
    -- running with ort
    -- ort: #1[A10s32x1024x1152[230.625,331.25:A282.0973632103867]]
    -- diff abs=0.25, rel=0.0009718135206471501, n=37748736.0/#1498>0.0-#1498>0.0001-#1498>0.001-#1498>0.01-#1498>0.1
...
.. rst-class:: sphx-glr-script-out
 ...
 .. code-block:: none
    ...
    provider dtype                                           ...                      
    CPU      FLOAT    0.000183  6.353993e-07      56.165009  ...       0       0     0
             FLOAT16  0.250000  9.765587e-04  147953.625000  ...  592527  592527     0
    CUDA     FLOAT    0.019501  6.741400e-05  127134.867172  ...  437722       0     0
             FLOAT16  0.250000  9.718135e-04     374.125000  ...    1498    1498     0
...
.. rst-class:: sphx-glr-script-out
 ...
 .. code-block:: none
    ...
    -- ORT-ORT: #1[CT1s2x1024x1152[238.43858337402344,325.4949645996094:A282.0361598214626]]
    -- diff with torch abs=0.00018310546875, rel=6.353993180060876e-07, n=2359296.0
    -- diff with ort abs=0, rel=0

    -- ORT-ORT running on CPU with FLOAT16
    -- ORT-ORT: #1[CT10s2x1024x1152[238.375,325.5:A282.03590138753253]]
    -- diff with torch abs=0.5, rel=0.0017889023652867075, n=2359296.0
    -- diff with ort abs=0.25, rel=0.0009765586853176355, n=2359296.0

    -- ORT-ORT running on CUDA with FLOAT
    -- ORT-ORT: #1[GT1s32x1024x1152[230.67393493652344,331.3173828125:A282.09724871351983]]
    -- diff with torch abs=0.019500732421875, rel=6.741399644447511e-05, n=37748736.0
    -- diff with ort abs=0, rel=0

    -- ORT-ORT running on CUDA with FLOAT16
    -- ORT-ORT: #1[GT10s32x1024x1152[230.625,331.25:A282.0973632103867]]
    -- diff with torch abs=0.25, rel=0.0009718135206471501, n=37748736.0
    -- diff with ort abs=0, rel=0

    -- ORT-TORCH running on CPU with FLOAT
    -- ORT-TORCH: #1[CT1s2x1024x1152[238.43856811523438,325.49493408203125:A282.03615984746205]]
    -- diff with torch abs=0.0001220703125, rel=4.540097350274697e-07, n=2359296.0
    -- diff with ort abs=0.000152587890625, rel=5.70750534926122e-07, n=2359296.0

    -- ORT-TORCH running on CPU with FLOAT16
    -- ORT-TORCH: #1[CT10s2x1024x1152[238.375,325.5:A282.0358993742201]]
    -- diff with torch abs=0.5, rel=0.0017889023652867075, n=2359296.0
    -- diff with ort abs=0.25, rel=0.0009765586853176355, n=2359296.0

    -- ORT-TORCH running on CUDA with FLOAT
    -- ORT-TORCH: #1[GT1s32x1024x1152[230.6787109375,331.31390380859375:A282.09982489121666]]
    -- diff with torch abs=0.000152587890625, rel=5.478333113210988e-07, n=37748736.0
    -- diff with ort abs=0.01947021484375, rel=6.74185413977852e-05, n=37748736.0

    -- ORT-TORCH running on CUDA with FLOAT16
    -- ORT-TORCH: #1[GT10s32x1024x1152[230.625,331.25:A282.0973632103867]]
    -- diff with torch abs=0.25, rel=0.0009718135206471501, n=37748736.0
    -- diff with ort abs=0, rel=0

    -- TORCH-ORT running on CPU with FLOAT
    -- TORCH-ORT: #1[CT1s2x1024x1152[238.43858337402344,325.4949645996094:A282.03615982539486]]
    -- diff with torch abs=0.00018310546875, rel=6.353993180060876e-07, n=2359296.0
    -- diff with ort abs=0.0001220703125, rel=4.263506547427781e-07, n=2359296.0

    -- TORCH-ORT running on CPU with FLOAT16
    -- TORCH-ORT: #1[CT10s2x1024x1152[238.375,325.5:A282.0359066327413]]
    -- diff with torch abs=0.25, rel=0.0009596892142448591, n=2359296.0
    -- diff with ort abs=0.25, rel=0.0009765586853176355, n=2359296.0

    -- TORCH-ORT running on CUDA with FLOAT
    -- TORCH-ORT: #1[GT1s32x1024x1152[230.67393493652344,331.3173828125:A282.0972485739294]]
    -- diff with torch abs=0.019500732421875, rel=6.741399644447511e-05, n=37748736.0
    -- diff with ort abs=0.001953125, rel=7.323939667845861e-06, n=37748736.0

    -- TORCH-ORT running on CUDA with FLOAT16
    -- TORCH-ORT: #1[GT10s32x1024x1152[230.625,331.25:A282.0973633262846]]
    -- diff with torch abs=0, rel=0
    -- diff with ort abs=0.25, rel=0.0009708700160387727, n=37748736.0

    -- TORCH-TORCH running on CPU with FLOAT
    -- TORCH-TORCH: #1[CT1s2x1024x1152[238.43856811523438,325.49493408203125:A282.03615981600404]]
    -- diff with torch abs=0, rel=0
    -- diff with ort abs=0.00018310546875, rel=6.353997217386374e-07, n=2359296.0

    -- TORCH-TORCH running on CPU with FLOAT16
    -- TORCH-TORCH: #1[CT10s2x1024x1152[238.375,325.5:A282.03590652677747]]
    -- diff with torch abs=0, rel=0
    -- diff with ort abs=0.25, rel=0.0009765586853176355, n=2359296.0

    -- TORCH-TORCH running on CUDA with FLOAT
    -- TORCH-TORCH: #1[GT1s32x1024x1152[230.6787109375,331.31390380859375:A282.099824885446]]
    -- diff with torch abs=0, rel=0
    -- diff with ort abs=0.019500732421875, rel=6.74185413977852e-05, n=37748736.0

    -- TORCH-TORCH running on CUDA with FLOAT16
    -- TORCH-TORCH: #1[GT10s32x1024x1152[230.625,331.25:A282.0973633262846]]
    -- diff with torch abs=0, rel=0
    -- diff with ort abs=0.25, rel=0.0009708700160387727, n=37748736.0
...
.. rst-class:: sphx-glr-script-out
 ...
 .. code-block:: none
    ...
    FLOAT   CPU      ORT-ORT      0.000000    0.000183
            ...
            CUDA     ORT-ORT      0.000000    0.019501
                     ORT-TORCH    0.019470    0.000153
                     TORCH-ORT    0.001953    0.019501
                     TORCH-TORCH  0.019501    0.000000
...
.. rst-class:: sphx-glr-timing
   ...
   **Total running time of the script:** (0 minutes 46.042 seconds)
